...
from table_actions import create_order, delete_order, update_order_status, get_all_orders, get_order_by_id, get_user, get_user_by_order_id
...
async def delete_existing_order(order_id: int):
    ...
    if user and user['role'] == 'admin':
        user_from_order = get_user_by_order_id(order_id)
        send_message('orders_operations', f"cancelled {user_from_order.username} {user_from_order.email}")
...
async def update_existing_order(order_id: int, order: OrderUpdate):
    ...
    if user and user['role'] == 'admin':
        ...
        if updated_order:
            user_from_order = get_user_by_order_id(order_id)
            send_message('orders_operations', f"{order.status} {user_from_order.username} {user_from_order.email}")
